...
plot(lrcoef, mlrcoef)
```
Most of the simple and multiple regression coefficients are close to 0 which means they are similar to each other. nox is the only outlier with the simple linear coefficient value being 31 and the multiple linear regression coefficent value being -10.  
```{r}
...
```
(c) pruning the tree does not improve test MSE since the pruned MSE was 4.922 which is the same as a regression tree.
```{r}
...
2. yes, there is a premium on houses in neihborhood three. Looking at the dummy variable, Hood_3, a house in neighborhood 3 is predicted to have a price $27,279 higher than a house in neighborhood 1 holding all the other variables constant.
...
```
4. yes, you could combine neighborhoods 1 and 2 into a single binary variable, and as my validation set approach shows, the test MSE for combining the neighborhoods has a slightly lower test MSE than with the seperated neighborhoods but the test MSE's are quite similar.
...
finbrmse = sqrt(sum((ytest-yhat.mean)^2)/nrow(catest))
cat('finbrmse: ',finbrmse,'\n')

```
Bart does not do as well as random forest or boosting looking at the validation RMSE, both RF and boosting have RMSE's of .237 where as Bart has RMSE of .250.
...
for (decay in decayList){
  Bnn = nnet(crim~.,data = Btrainval,size=50,decay=decay,linout=T)
  pBnn = predict(Bnn, newdata = Btest)
  cv.rmse = sqrt(sum((Btest$crim-pBnn)^2)/nrow(Btest))
  cat("rmse:", cv.rmse, "decay =", decay, "\n")
}
```
```{r}
sizeList = c(1, 3, 5, 10, 25, 50)
for (size in sizeList){
  Bnn = nnet(crim~.,data = Btrainval,size=size,decay=.001,linout=T)
  pBnn = predict(Bnn, newdata = Btest)
  cv.rmse = sqrt(sum((Btest$crim-pBnn)^2)/nrow(Btest))
  cat("rmse:", cv.rmse, "size =", size, "\n")
}
```
after cross validating for a few decay and size parameters, found that the best decay value for size 50 was .3 with a test RMSE of 8.44. the best size for a decay parameter of .001 was size 50. Just like in the slides, it is probably more important to CV for the decay parameter and just have a large size as default. 
